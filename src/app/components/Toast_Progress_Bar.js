...
export default function ToastWithProgressBar({clicked}) {
  return (
    <section className="bg-gray-2 ">
      <div className="mx-auto px-4 sm:container">
        <div className="flex justify-end p-2">
          <div className="relative flex w-full max-w-[460px] items-center border-2 border-blue-400 overflow-hidden rounded-lg bg-white px-5 py-[18px] shadow-1 ">
            <span className="absolute bottom-0 left-0 block h-[3px] w-full bg-stroke ">
              <span className="absolute bg-blue-500 left-0 top-0 h-[3px] w-3/4 bg-primary"></span>
            </span>
            <span className="mr-4 flex h-[30px] w-full max-w-[30px] items-center justify-center rounded-full border border-primary">
              <svg
                width={16}
                height={16}
                viewBox="0 0 16 16"
            ...
            <button className="absolute right-5 top-1/2 -translate-y-1/2 text-dark-5 hover:text-primary " onClick={()=> clicked()}>
              <svg
                width={16}
                height={16}
                viewBox="0 0 16 16"
